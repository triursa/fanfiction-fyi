...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  ...
  if (targetUser.role === UserRole.Founder) {
    return new Response(JSON.stringify({ error: 'Cannot modify founder' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
  }

  await db.update(users)
    .set({ suspendedUntil: null, updatedAt: sql`(datetime('now'))` })
    .where(eq(users.id, userId));

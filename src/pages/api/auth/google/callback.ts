...
export const GET: APIRoute = async ({ url, locals, request }) => {
  ...
  if (state && state.startsWith('link_')) {
    ...
    const oauthState = await queryFirst<any>(db, `SELECT * FROM oauth_states WHERE state = ?1 AND user_id = ?2`, state, linkUserId);
    if (!oauthState) {
      return Response.redirect(`${url.origin}/settings?error=invalid_link_state`, 302);
    }
    // Consume the state (one-time use) and clean up expired states
    await run(db, `DELETE FROM oauth_states WHERE state = ?1`, state);
    await run(db, `DELETE FROM oauth_states WHERE created_at < datetime('now', '-10 minutes')`);
